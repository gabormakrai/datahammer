...
public class Process extends CommandLine {
	...
	// LoggerFactory.getLogger(DataHammerProcess.class);
	//
	...
	//
	// protected final HashMap<String, String> defaultParameter = new
	...
	// //
	// }
	// protected void preprocessData(DataType dataType, HashMap<String, String>
	...
	// String[] methods = method.substring(0, method.length() -
	// 1).substring(method.indexOf("[") + 1).split("\\,");
	// model.apply(table);
	// result = new EvaluationResult[methods.length];
	// for (int i=0;i<methods.length;i++) {
	// String m = methods[i];
	// AbstractEvaluator eval =
	// ClassificationEvaluationFactory.createEvaluator(m);
	// if (eval == null) {
	...
	// }
	//
	// public double runXValidation(HashMap<String, String> parameters) throws
	// Throwable {
	//
	...
	// try {
	// crossValidationCycle =
	// Integer.parseInt(parameters.get(XVALIDATION_CYCLE));
	// } catch (NumberFormatException e) {
	...
	//
	// xValidationMarking(trainTable, "" + crossValidationCycle, "" +
	// xValidation, "true", "42");
	//
	// learn(parameters);
	//
	...
	// finalRes = res;
	// }
	// silentLogger.log("Average " + results[0][i].getEvaluationMethodName() +
	// ": " + res);
	// }
	//
	// return finalRes;
