...
public class FloatDataColumn extends AbstractDataColumn {
	...
	public void merge(AbstractDataColumn column) {
		if (!(column instanceof FloatDataColumn)) {
			throw new RuntimeException("Column " + column + " is not compatible with this column " + this);
		}
		
		FloatDataColumn otherColumn = (FloatDataColumn)column;
		...
		float[] newData = new float[data.length + otherColumn.data.length];
